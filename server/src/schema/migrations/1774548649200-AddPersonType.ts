...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "asset_face" ADD COLUMN IF NOT EXISTS "type" "person_type_enum" NOT NULL DEFAULT 'human'`.execute(db);
}

export async function down(db: Kysely<any>): Promise<void> {
  await sql`ALTER TABLE "asset_face" DROP COLUMN IF EXISTS "type"`.execute(db);
